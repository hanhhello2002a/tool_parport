...
# ==================== ĐỒNG BỘ VERSION VÀ GITHUB ====================
# Để 0.0 để luôn thấp hơn GitHub, ép Tool tự động hiện bảng cập nhật lần đầu nếu chưa đồng bộ
BASE_VERSION = "0.0"  
...
# 🟢 ĐƯỜNG DẪN GỐC (Vị trí thư mục chứa file .py hoặc file .exe thực tế ngoài ổ cứng)
if hasattr(sys, 'frozen'):
    ROOT_DIR = os.path.dirname(sys.executable)       # Thư mục chứa file .exe thực tế
else:
    ROOT_DIR = os.path.dirname(os.path.abspath(__file__)) # Thư mục chứa file .py thô
...
class PassportToolGUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        # 1. Nạp cấu hình từ máy lên trước (Lấy số version thực tế đã lưu ở máy)
        self.local_version = BASE_VERSION 
        self.output_dir_var = tk.StringVar()
        self.prefix_num_var = tk.StringVar()
        self.suffix_var = tk.StringVar()
        self.load_config()
        
        # 2. Thiết lập tiêu đề hiển thị đúng Version đã đồng bộ
        self.title(f"UK PASSPORT TOOL v{self.local_version}")
        self.geometry("950x800")
        
        # 🟢 TỰ ĐỘNG ĐỌC LOGO NỘI BỘ (TỪ RUỘT PYINSTALLER) HOẶC NGOÀI FILE
        try:
            if hasattr(sys, '_MEIPASS'):
                icon_path = os.path.join(sys._MEIPASS, "logo/logo.ico")
            else:
                icon_path = os.path.join(ROOT_DIR, "logo/logo.ico")
        ...
        # 🟢 ĐỒNG BỘ ĐƯỜNG DẪN PSD: Ưu tiên file ngoài cạnh EXE (nếu đã update), ngược lại dùng file trong ruột EXE
        self.refresh_psd_path()
        
        self.placeholder_sample = (
            "hovaten,ho,ho2,ten,ten2,so,so2,ntns,ntns2,ma1,ma1.2,ma2,ma2.2\n"
        )
        
        # 3. Tạo các Tab tính năng chính
        ...
        # 4. Kích hoạt lắng nghe thay đổi ô nhập để tự động lưu cấu hình
        self.setup_trace_listeners()
        
        self.remote_config_data = {} 
        self.after(200, self.check_for_updates)

    def refresh_psd_path(self):
        """Xác định đường dẫn file PSD động: ưu tiên file ngoài cạnh EXE, nếu không có lấy file đính kèm trong ruột"""
        external_psd = os.path.join(ROOT_DIR, "design.psd")
        
        if os.path.exists(external_psd):
            self.psd_path = external_psd
        elif hasattr(sys, '_MEIPASS'):
            self.psd_path = os.path.join(sys._MEIPASS, "design.psd")
        else:
            self.psd_path = external_psd

    def setup_trace_listeners(self):
        self._trace_p = self.prefix_num_var.trace_add("write", lambda *args: self.save_config())
        self._trace_s = self.suffix_var.trace_add("write", lambda *args: self.save_config())
        self._trace_o = self.output_dir_var.trace_add("write", lambda *args: self.save_config())

    def remove_trace_listeners(self):
        try:
            self.prefix_num_var.trace_remove("write", self._trace_p)
            self.suffix_var.trace_remove("write", self._trace_s)
            self.output_dir_var.trace_remove("write", self._trace_o)
        except: pass
    ...
    def check_for_updates(self):
        def async_check():
            try:
                ...
                remote_version_str = str(self.remote_config_data.get("version", self.local_version)).strip()
                
                # So sánh bản đám mây với phiên bản thực tế được ghi nhận ở máy người dùng
                if float(remote_version_str) > float(self.local_version):
                    self.show_update_overlay(remote_version_str)
            except:
                pass
    ...
    def show_update_overlay(self, new_version):
        ...
        lbl_title = ctk.CTkLabel(self.overlay, text="⚡ ĐỒNG BỘ PHÔI PSD & CẤU HÌNH MỚI", font=("Consolas", 15, "bold"), text_color="#ef4444")
        lbl_title.pack(pady=(220, 10))
        lbl_info = ctk.CTkLabel(self.overlay, text=f"Hệ thống phát hiện tài nguyên thiết kế hoặc cấu hình chuỗi MA2 mới trên GitHub (v{new_version}).\nỨng dụng sẽ tự động tải phôi mới về nằm cùng thư mục chứa file chạy của bạn.", font=("Consolas", 12), text_color="#a3a3a3")
        ...
        self.btn_update = ctk.CTkButton(self.overlay, text="📥 ĐỒNG BỘ TÀI NGUYÊN NGAY", font=("Consolas", 14, "bold"), height=45, fg_color="#22c55e", hover_color="#16a34a", text_color="#000000", command=lambda: threading.Thread(target=self.execute_resource_update).start())
    ...
    def execute_resource_update(self):
        ...
        try:
            # 1. Tắt Photoshop chạy ngầm để giải phóng file tránh bị lỗi kẹt khi ghi đè ngoài ổ cứng
            self.progress_lbl.configure(text="⏳ Đang giải phóng ứng dụng Photoshop ngầm...", text_color="#eab308")
            os.system("taskkill /f /im Photoshop.exe >nul 2>&1")
            
            # 2. Tải trực tiếp file phôi thiết kế mới về nằm CÙNG THƯ MỤC ROOT_DIR thực tế
            self.progress_lbl.configure(text="⏳ Đang tải phôi thiết kế design.psd mới nhất...", text_color="#06b6d4")
            psd_url = f"{RAW_REPO_URL}design.psd?nocache={random.randint(100000, 999999)}"
            target_psd_path = os.path.join(ROOT_DIR, "design.psd")
            ...
            # 3. Đọc dữ liệu biến đổi từ GitHub 
            self.progress_lbl.configure(text="⚙️ Đang đồng bộ chuỗi cấu hình MA2 từ Server...", text_color="#c084fc")
            new_prefix = self.remote_config_data.get("prefix_num", "0")
            new_suffix = self.remote_config_data.get("suffix", "1M3503029<<<<<<<<<<<<<<04")
            new_fixed_number = self.remote_config_data.get("fixed_number", "3698531423")
            new_version = str(self.remote_config_data.get("version", self.local_version)).strip()
            
            # Tạm thời gỡ bỏ listener tránh xung đột dữ liệu cũ đè ngược lên file config mới
            self.remove_trace_listeners()

            # Lưu đè dữ liệu mới kèm số hiệu Version mới nhất xuống thư mục ROOT_DIR thực tế
            with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                json.dump({
                    "version": new_version,
                    "prefix_num": str(new_prefix).strip(),
                    "suffix": str(new_suffix).strip(),
                    "output_dir": self.output_dir_var.get().strip(),
                    "fixed_number": str(new_fixed_number).strip()
                }, f, ensure_ascii=False, indent=4)
                
            self.progress_lbl.configure(text="✅ Đã cập nhật tài nguyên thành công!", text_color="#22c55e")
            messagebox.showinfo("Thành công", f"Đã cập nhật phôi thiết kế và đồng bộ hệ thống lên phiên bản v{new_version} thành công!\nGiao diện sẽ tự động làm mới ngay bây giờ.")
            
            # 4. Tải lại cấu hình mới lên RAM và làm mới giao diện tại chỗ
            self.load_config()          
            self.title(f"⚡ UK PASSPORT TOOL v{self.local_version}")
            self.refresh_psd_path() # Cập nhật lại đường dẫn PSD trỏ ra file ngoài vừa tải về
            self.setup_trace_listeners() 
            ...
            self.setup_trace_listeners()
            messagebox.showerror("Lỗi Cập Nhật", f"Không thể đồng bộ tài nguyên tự động.\nChi tiết: {e}")
    ...
    def load_config(self):
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.local_version = data.get("version", BASE_VERSION)
                    self.prefix_num_var.set(data.get("prefix_num", "0"))
                    self.suffix_var.set(data.get("suffix", "1M3503029<<<<<<<<<<<<<<04"))
                    saved_dir = data.get("output_dir", ROOT_DIR)
                    if os.path.exists(saved_dir):
                        self.output_dir_var.set(os.path.normpath(saved_dir))
                    else:
                        self.output_dir_var.set(ROOT_DIR)
                    
                    # Load fixed number if exists
                    self.fixed_number = data.get("fixed_number", "3698531423")
                    if hasattr(self, 'ent_fixed_num'):
                        self.ent_fixed_num.delete(0, tk.END)
                        self.ent_fixed_num.insert(0, self.fixed_number)
                    return
            except: pass
        ...
        self.output_dir_var.set(ROOT_DIR)
        self.fixed_number = "3698531423"
        if hasattr(self, 'ent_fixed_num'):
            self.ent_fixed_num.delete(0, tk.END)
            self.ent_fixed_num.insert(0, "3698531423")
    ...
    def save_config(self):
        try:
            config_data = {
                ...
                "output_dir": self.output_dir_var.get().strip(),
                "fixed_number": self.ent_fixed_num.get().strip() if hasattr(self, 'ent_fixed_num') else "3698531423"
            }
            with open(CONFIG_FILE, "w", encoding="utf-8") as f:
    ...
    def process_photoshop(self):
        ...
        # Cập nhật chính xác vị trí file PSD trước khi nạp vào Photoshop
        self.refresh_psd_path()
        
        if not os.path.exists(self.psd_path):
            messagebox.showerror("Error", f"Không tìm thấy file phôi thiết kế tại:\n{self.psd_path}")
        ...
        try:
            ...
            messagebox.showerror("Photoshop Error", f"Không thể kết nối với phần mềm Photoshop cài trên máy.\nChi tiết: {e}")
            self.btn_run.configure(state="normal", fg_color="#22c55e", text_color="#000000")
            self.update_status("Ready", "#22c55e")
            pythoncom.CoUninitialize()
            return
    ...
    def init_tab_generate(self):
        ...
        # Số cố định trước GBR
        ctk.CTkLabel(frame_config, text="Số cố định trước GBR:", font=("Consolas", 12)).grid(row=1, column=0, sticky="w", padx=20, pady=5)
        self.ent_fixed_num = ctk.CTkEntry(frame_config, width=200, font=("Consolas", 12))
        self.ent_fixed_num.insert(0, self.fixed_number if hasattr(self, 'fixed_number') else "3698531423")
        self.ent_fixed_num.grid(row=1, column=1, sticky="w", padx=5, pady=5)
        
        # Ghi chú hiển thị cấu trúc MA2
        ctk.CTkLabel(frame_config, text="→ Cấu trúc MA2: [SỐ CỐ ĐỊNH] + GBR + [DOB] + [SUFFIX]", font=("Consolas", 10), text_color="#888888").grid(row=1, column=2, sticky="w", padx=10, pady=5)
    ...
    def generate_random_data(self):
        ...
        count_str = self.ent_count.get().strip()
        
        # Lấy số cố định từ ô nhập liệu
        fixed_number = self.ent_fixed_num.get().strip()
        fixed_suffix = self.suffix_var.get().strip()
        
        if not fixed_number:
            fixed_number = "3698531423"  # Giá trị mặc định nếu để trống
        
        if not count_str.isdigit():
            messagebox.showerror("Error", "Số lượng dòng phải là số nguyên!")
            return
            
        self.lbl_gen_status.configure(text="Đang tạo dữ liệu...", text_color="#06b6d4")
        
        try:
            url = f"https://randomuser.me/api/?results={count_str}&nat=gb&gender={gender}"
            ...
            for user in results:
                ...
                year_birth = random.randint(1970, 2000)
                m_idx = random.randint(0, 11)
                d_val = random.randint(1, 28)
                
                # Định dạng DOB cho MRZ (YYMMDD)
                dob = f"{str(year_birth)[-2:]}{str(m_idx + 1).zfill(2)}{str(d_val).zfill(2)}"
                
                # Cấu trúc MA2: [Số cố định] + GBR + [DOB] + [Suffix cố định]
                m2 = f"{fixed_number}GBR{dob}{fixed_suffix}"
                
                # Định dạng MA1 và các thông tin khác
                ho_part = ho.replace(" ", "<")
                ...
                # Thêm vào hàng dữ liệu
                row_str = f'"{ho} {ten}","{ho}","{ho}","{ten}","{ten}","{fixed_number}","{fixed_number}","{ntns_format}","{ntns_format}","{ma1}","{ma1}","{m2}","{m2}"'
                rows.append(row_str)
                
            self.txt_gen_out.delete("1.0", "end")
            self.txt_gen_out.insert("1.0", "\n".join(rows))
            self.lbl_gen_status.configure(text="Đã tạo thành công dữ liệu với số cố định!", text_color="#22c55e")
            
        except Exception as e:
            self.lbl_gen_status.configure(text=f"Lỗi: {str(e)}", text_color="#ef4444")
    ...
    def generate_perfect_mrz_data(self):
        ...
        # Lấy số cố định từ tab cấu hình
        if not hasattr(self, 'ent_fixed_num'):
            fixed_number = "3698531423"
        else:
            fixed_number = self.ent_fixed_num.get().strip()
            if not fixed_number:
                fixed_number = "3698531423"
        ...
        try:
            ...
            for _ in range(count):
                ...
                # Cấu trúc MA2 mới với số cố định trước GBR
                c1 = self.mrz_cd(pNum)
                c2 = self.mrz_cd(dob)
                c3 = self.mrz_cd(exp)
                opt = "<<<<<<<<<<<<<<" 
                ...
                # Tạo phần đầu với số cố định
                part2 = f"{fixed_number}GBR{dob}{c2}{sex_sel}{exp}{c3}{opt}{c4}"
            ...
            self.lbl_status_p.configure(text=f"Compile Error: {str(e)}", text_color="#ef4444")
