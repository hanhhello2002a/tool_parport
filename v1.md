...
# Đây chỉ là phiên bản gốc cơ sở khi máy chưa có file config.json
BASE_VERSION = "3.9"  
...
class PassportToolGUI(ctk.CTk):
    def __init__(self):
        ...
        # 🟢 THÊM DÒNG NÀY ĐỂ HIỂN THỊ LOGO Ở GÓC TRÁI TRÊN CÙNG
        try:
    ...
    def execute_resource_update(self):
        ...
        try:
            ...
            # 1. Tắt Photoshop chạy ngầm để giải phóng file design.psd tránh bị lỗi kẹt khi ghi đè
            ...
            # 2. Tải trực tiếp file phôi thiết kế mới từ GitHub về đè lên file cũ
            ...
            # Lưu đè dữ liệu mới kèm số hiệu Version mới nhất từ GitHub xuống máy người dùng
            ...
            # 4. Tải lại cấu hình mới lên RAM và làm mới giao diện tại chỗ (Không cần tắt file .exe)
            self.load_config()          
            self.title(f"⚡ UK PASSPORT - PROTOCOL TOOL v{self.local_version}")
            self.setup_trace_listeners() 
    ...
    def generate_random_data(self):
        ...
        try:
            ...
            for user in results:
                ...
                # [ĐÃ SỬA CHUẨN XÓA TYPO]: Đổi "ma1}" thành thiết lập f-string hợp lệ "{ma1}"
